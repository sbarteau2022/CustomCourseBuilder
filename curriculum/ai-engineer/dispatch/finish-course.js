// CANONICAL recovery script — use when dispatch-course.js's background run
// authored some packets but stalled/errored before gating all of them
// (this happened during the foundation-tier fan-out: a session limit and,
// separately, a background workflow that stopped returning results).
//
// Finish ONE course: author only its MISSING packets (check
// curriculum/ai-engineer/materials/<CODE>/ for what's already there), then
// four-gate EVERY packet in the course, including ones already written
// (they haven't necessarily been gated yet either). Edit SELECTED and
// ALREADY_WRITTEN per run — same COURSES map / emit-course-snippet.ts
// workflow as dispatch-course.js.
//
// If even THIS stalls on a handful of packets, don't keep waiting on a
// background run indefinitely — dispatch a small, targeted
// fix-and-gate/gate-only Workflow for just the stragglers (see the git
// history of this repo's foundation-tier PRs for worked examples of that
// pattern: fix the flagged defect, then re-gate, both in one small script).
export const meta = {
  name: 'finish-course-materials',
  description: 'Author remaining packets for one course, then four-gate every packet in it',
  phases: [
    { title: 'Author', detail: 'one agent per MISSING packet writes full lesson material to disk' },
    { title: 'Gate', detail: 'four-gate review per packet (all packets in the course)' },
  ],
}

// ─────────── EDIT PER RUN ───────────
const SELECTED = 'AIE-101'
const ALREADY_WRITTEN = new Set(['AIE-101-M01','AIE-101-M02','AIE-101-M03','AIE-101-M04'])
// ─────────────────────────────────────
...
const allPackets = course.packets
const missingPackets = allPackets.filter((p) => !ALREADY_WRITTEN.has(p.id))
...
log(`Finishing ${course.code}: authoring ${missingPackets.length} missing packets, then four-gating all ${allPackets.length}`)

// Author only what's missing (pipeline: no barrier needed, but small set).
